...
export default function Navbar() {
  ...
  const [productMenu, setProductMenu] = useState<any>(null);
    ...
    const fetchCategories = async () => {
      try {
        ...
        if (catData.success && catData.data) {
          ...
          const mainCats = allCats.filter((c: any) => !c.parent);
          const subCats = allCats.filter((c: any) => c.parent);

          setProductMenu({
            label: "Products",
            mainCategories: mainCats.map((main: any) => ({
              ...main,
              subCategories: subCats.filter((sub: any) => String(sub.parent) === String(main._id))
            }))
          });
        }
      } catch (error) {
  ...
  const getLinkClasses = (path: string) => {
    ...
    return `group relative text-[13px] lg:text-[14px] tracking-[0.1em] font-medium uppercase transition-colors py-2 ${active ? "text-[#C89B3C]" : "text-neutral-700 hover:text-[#C89B3C]"}`;
  };
  ...
  return (
    <nav className="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-sm" style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}>
      <div className="w-full  px-4 sm:px-8 md:px-12 lg:px-20 xl:px-24 mx-auto">
        <div className="flex justify-between items-center h-24">
          
          {/* 1. Logo (Left Aligned - Fixed minimum width to balance layout) */}
          <div className="flex mr-[96px] items-center lg:w-[220px]">
          ...
          <div className="hidden lg:flex flex-1 justify-center items-center space-x-6 xl:space-x-10 h-full">
            ...
            {/* 🔥 FIX: Products menu is ALWAYS rendered so it doesn't jump. Only the dropdown waits for data. */}
            <div 
              className="relative h-full flex items-center group cursor-pointer"
              onMouseLeave={() => setActiveMainIndex(null)}
            >
              <div className={`group/dropdown relative flex items-center gap-1.5 text-[13px] lg:text-[14px] tracking-[0.1em] font-medium uppercase transition-colors py-2 ${isProductsActive ? "text-[#C89B3C]" : "text-neutral-700 hover:text-[#C89B3C]"}`}>
                <PackageSearch size={18} className="text-[#C89B3C]" />
                PRODUCTS
                <ChevronDown size={16} className={`mt-0.5 transition-transform duration-300 group-hover:rotate-180 ${isProductsActive ? "text-[#C89B3C]" : "text-neutral-400"}`} />
              ...
              {productMenu && productMenu.mainCategories && (
                <div className="absolute top-[calc(100%-1px)] left-1/2 -translate-x-1/2 w-[280px] bg-white shadow-md border border-gray-100 py-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                  {productMenu.mainCategories.map((main: any, idx: number) => (
          ...
          <div className="flex items-center justify-end lg:w-[220px]">
            ...
            <div className="lg:hidden flex items-center ml-4">
              <button onClick={() => setIsMobileOpen(!isMobileOpen)} className="text-neutral-800 hover:text-[#C89B3C] transition-colors">
      ...
      {/* Mobile Menu */}
      {isMobileOpen && (
        <div className="lg:hidden bg-white border-t border-gray-100 absolute w-full shadow-lg h-screen overflow-y-auto pb-24">
          <div className="px-6 py-6 space-y-4">
            ...
            <div className="py-3">
              ...
              {productMenu?.mainCategories?.map((main: any) => (
