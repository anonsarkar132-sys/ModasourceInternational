          ...
          setProductMenu({
            ...
            mainCategories: mainCats.map((main) => ({
              ...
              subCategories: subCats.filter(
                (sub) => String(sub.parent) === String(main._id),
              ),
            })),
          });
        }
        ...
        <div className="flex justify-between items-center h-20 md:h-24">
          ...
          <div className="hidden xl:flex flex-1 justify-center items-center gap-4 2xl:gap-8 h-full min-w-0">
            <Link
              ...
              className={getLinkClasses(navLinks[0].href)}
            >
              {navLinks[0].label}
            ...
            <Link
              ...
              className={getLinkClasses(navLinks[1].href)}
            >
              {navLinks[1].label}
            ...
            >
              <Link href="/products" className={getLinkClasses("/products")}>
                <div
                  className={`group/dropdown relative flex items-center gap-1.5 whitespace-nowrap text-[12px] 2xl:text-[14px] tracking-[0.08em] 2xl:tracking-[0.1em] font-medium uppercase transition-colors py-2 ${isProductsActive ? "text-[#C89B3C]" : "text-neutral-700 hover:text-[#C89B3C]"}`}
                >
                  <PackageSearch
                    size={17}
                  ...
                  <ChevronDown
                    size={15}
                    className={`mt-0.5 shrink-0 transition-transform duration-300 group-hover:rotate-180 ${isProductsActive ? "text-[#C89B3C]" : "text-neutral-400"}`}
                  />
                  <span
                    className={`absolute bottom-0 left-0 w-full h-[2px] bg-[#C89B3C] transition-transform origin-left duration-300 ${isProductsActive ? "scale-x-100" : "scale-x-0 group-hover/dropdown:scale-x-100"}`}
                  ></span>
                </div>
              </Link>
                  ...
                  {productMenu.mainCategories.map((main, idx) => (
                    ...
                    >
                      <Link
                        ...
                        className="flex justify-between items-center px-6 py-3.5 text-[13px] tracking-widest text-neutral-600 hover:text-[#C89B3C] hover:bg-neutral-50 uppercase transition-colors"
                      >
                        {main.name}
                          ...
                          <div className="absolute top-0 left-full w-[260px] bg-white shadow-md border border-gray-100 py-3 animate-in fade-in slide-in-from-left-2">
                            {main.subCategories.map((sub) => (
                              <Link
                                ...
                                className="block px-6 py-3.5 text-[13px] tracking-widest text-neutral-500 hover:text-[#C89B3C] hover:bg-neutral-50 uppercase transition-colors"
                              >
                                {sub.label || sub.name}
                              </Link>
                            ))}
            ...
            <Link
              ...
              className={getLinkClasses(navLinks[2].href)}
            >
              {navLinks[2].label}
            ...
            <Link
              ...
              className={getLinkClasses(navLinks[3].href)}
            >
              {navLinks[3].label}
            ...
            <Link
              ...
              className={getLinkClasses(navLinks[4].href)}
            >
              {navLinks[4].label}
          ...
          <div className="flex items-center justify-end xl:w-[180px] 2xl:w-[220px]">
            <div className="hidden xl:flex items-center gap-3">
              <a
                href="#"
                className="w-9 h-9 rounded-full bg-neutral-800 text-white flex items-center justify-center hover:bg-[#C89B3C] transition-colors"
              >
                <LinkedinIcon />
              </a>
              <a
          ...
          <div className="px-5 sm:px-8 py-6 space-y-4">
            {navLinks.slice(0, 2).map((item, index) => (
              <Link
                ...
                className={`block py-3 text-base uppercase tracking-widest font-medium border-b border-gray-50 transition-colors ${isActive(item.href) ? "text-[#C89B3C]" : "text-neutral-800 hover:text-[#C89B3C]"}`}
              >
                {item.label}
              </Link>
            ))}
              ...
              {productMenu?.mainCategories?.map((main) => (
                <div key={main._id} className="mb-5 pl-2">
                  <Link
                    href={`/products/${main.slug}`}
                    onClick={() => setIsMobileOpen(false)}
                    className={`block font-semibold uppercase text-sm tracking-wider mb-3 transition-colors ${pathname === `/products/${main.slug}` ? "text-[#C89B3C]" : "text-neutral-800 hover:text-[#C89B3C]"}`}
                  >
                    {main.name}
                  </Link>
                  <div className="pl-5 space-y-3 border-l-2 border-neutral-100">
                    {main.subCategories?.map((sub) => (
                      <Link
                        ...
                        href={`/products/${sub.slug}`}
                        onClick={() => setIsMobileOpen(false)}
                        className={`block text-sm uppercase tracking-widest py-1 transition-colors ${pathname === `/products/${sub.slug}` ? "text-[#C89B3C] font-semibold" : "text-neutral-500 hover:text-[#C89B3C]"}`}
                      >
                        {sub.name}
                      </Link>
                    ))}
